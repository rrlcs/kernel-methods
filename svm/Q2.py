...
def cvxopt_solver():
    ...
    P, q, G, h = get_opt_var(0.0001) ## Change Hyperparameter C
...
def get_y_pred(W_opt, b_opt, X, n):
    ...
    y_pred = get_labels_of_xi(W_opt.T, b_opt, X, n)
    return y_pred
...
if __name__ == '__main__':
    ...
    y = get_labels_of_xi(W, b, X, n_train)

    ''' Saving Training Dataset '''
    np.save('Data_X_Q2_a.npy', X)
    np.save('Data_y_Q2_a.npy', y)


    ''' Sample Test Data '''
    X_test = get_samples_of_xi(n_test)
    y_test = get_labels_of_xi(W, b, X_test, n_test)
